...
package catalogoempresa.accesoadatos;
...
public class RolDAL {
  static String obtenerCampos() {
    ...
    private static String obtenerSelect(Rol pRol) {
        ...
        sql += (obtenerCampos() + " FROM Rol r"); // Forma un metodo SELECT 
        return sql;
    }
    
    private static String agregarOrderBy(Rol pRol) { //Ordena 
    ...
    public static int crear(Rol pRol) throws Exception {
        ...
        try (Connection conn = ComunDB.obtenerConexion();) { 
            sql = "INSERT INTO Rol(Nombre) VALUES(?)"; // el insert modifica 
            try (PreparedStatement ps = ComunDB.createPreparedStatement(conn, sql);) {
                ps.setString(1, pRol.getNombre()); // aqui se coloca de cuantos parametros tendriamos 
